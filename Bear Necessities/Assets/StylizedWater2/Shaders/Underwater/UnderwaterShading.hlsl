...
void MixUnderwaterReflections(inout float3 color, in float3 sceneColor, float skyMask, float3 positionWS, float3 normalWS, float3 worldTangentNormal, float3 viewDir, float2 screenPos, int vFace, float density)
{
...
float3 ShadeUnderwaterSurface(in float3 albedo, float3 emission, float3 specular, float3 sceneColor, float skyMask, float shadowMask, float3 positionWS, float3 normalWS, float3 worldTangentNormal, float3 viewDir, float2 screenPos, float3 shallowColor, float3 deepColor, int vFace)
{
	...
	MixUnderwaterReflections(color.rgb, sceneColor.rgb, skyMask, positionWS, normalWS, worldTangentNormal, viewDir, screenPos, vFace, 1-density);
	#endif
