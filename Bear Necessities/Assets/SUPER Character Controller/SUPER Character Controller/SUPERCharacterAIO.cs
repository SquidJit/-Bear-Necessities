...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    public bool enableGroundingDebugging = false, enableMovementDebugging = false, enableMouseAndCameraDebugging = false, enableVaultDebugging = false;
        ...
        void Awake()
        {
            m_Animator = transform.Find("Bear").GetComponent<Animator>();
    ...
    void FixedUpdate() {
        if(!controllerPaused){
            ...
            if(enableMovementControl){
                ...
                MovePlayer(InputDir,currentGroundSpeed);
                    ...
                    //Mathf.Max(verticalAxis, horizontalAxis);
                    //speedAxis = axisArray.Select(Mathf.Abs).Max();
                    //Mathf.Abs(axisArray);
                    //speedAxis = axisArray.Max(Mathf.Abs);
                    verticalAxis = Mathf.Abs(verticalAxis);
                    horizontalAxis = Mathf.Abs(horizontalAxis);
                    speedAxis = Mathf.Max(verticalAxis, horizontalAxis);

                    setSpeedFloat(speedAxis);
                    m_Animator.SetBool("Moving", (verticalAxis + horizontalAxis >= .2));
        ...
        private void setSpeedFloat(float axis)
        {
            m_Animator.SetFloat("ForwardSpeed", axis);
